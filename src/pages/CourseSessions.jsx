...
const CourseSessions = () => {
  ...
  const [newExam, setNewExam] = useState({
    examName: '',
    questions: [{ question: '', alternatives: [{ description: '', isTrue: false }] }],
  });
  ...
  return (
    <div
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', paddingTop: 40, paddingLeft: 40 }}>
        <Button variant='link' onClick={() => navigate(-1)} style={{ marginBottom: 24 }}>
          ← Voltar
        </Button>
            ...
            sessions.map((session, idx) => (
              <Card
                ...
                <Card.Body>
                  ...
                  <Card.Text style={{ fontSize: 14, color: '#888' }}>
                    {session.description}
                  </Card.Text>
                  <hr />
                  {/* Materiais */}
                  <div style={{ marginBottom: 8 }}>
                    ...
                    <ul style={{ paddingLeft: 18 }}>
                      {session.materiais.length > 0 ? (
                        session.materiais.map((mat, i) => (
                          <li key={i} style={{ fontSize: 14 }}>
                            ...
                            <span
                              style={{ color: '#c00', cursor: 'pointer', marginLeft: 8 }}
                              onClick={() => handleRemoveItem(idx, 'materiais', i)}>
                              ×
                            </span>
                          </li>
                        ))
                    ...
                    <Button
                      ...
                      onClick={() => handleShowItemModal(idx, 'materiais')}>
                      + Material
                    </Button>
                  </div>
                  {/* Aulas */}
                  <div style={{ marginBottom: 8 }}>
                    ...
                    <ul style={{ paddingLeft: 18 }}>
                      {session.aulas.length > 0 ? (
                        session.aulas.map((aula, i) => (
                          <li key={i} style={{ fontSize: 14 }}>
                            <b>{aula.name}</b>
                            {aula.url && (
                              <>
                            ...
                            <span
                              style={{ color: '#c00', cursor: 'pointer', marginLeft: 8 }}
                              onClick={() => handleRemoveItem(idx, 'aulas', i)}>
                              ×
                            </span>
                          </li>
                        ))
                    ...
                    <Button
                      size='sm'
                      variant='outline-primary'
                      onClick={() => handleShowItemModal(idx, 'aulas')}>
                      + Aula
                    </Button>
                  </div>
                  {/* Exames */}
                  <div style={{ marginBottom: 8 }}>
                    ...
                    <ul style={{ paddingLeft: 18 }}>
                      {session.exames.length > 0 ? (
                        session.exames.map((exame, i) => (
                          <li key={i} style={{ fontSize: 14 }}>
                            <b>{exame.examName}</b>
                                  ...
                                  <li key={qi}>
                                    ...
                                    <ul style={{ fontSize: 13, marginTop: 2 }}>
                                      {q.alternatives.map((alt, ai) => (
                                        <li
                                          key={ai}
                                          style={{ color: alt.isTrue ? '#2e7d32' : '#555' }}>
                            ...
                            <span
                              style={{ color: '#c00', cursor: 'pointer', marginLeft: 8 }}
                              onClick={() => handleRemoveItem(idx, 'exames', i)}>
                              ×
                            </span>
                          </li>
                        ))
                    ...
                    <Button
                      size='sm'
                      variant='outline-primary'
                      onClick={() => handleShowItemModal(idx, 'exames')}>
                      + Exame
                    </Button>
                  </div>
                  <hr />
        ...
        <Modal show={showItemModal} onHide={() => setShowItemModal(false)}>
          <Modal.Header closeButton>
            <Modal.Title>
              Adicionar{' '}
              {itemType === 'materiais' ? 'Material' : itemType === 'aulas' ? 'Aula' : 'Exame'}
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <Form>
                          ...
                          questions: [
                            {
                              ...
                              alternatives: newExam.questions[0].alternatives,
                            },
                          ],
                        })
                      }
                  ...
                  {newExam.questions[0].alternatives.map((alt, idx) => (
                    <div key={idx} className='mb-2 d-flex align-items-center' style={{ gap: 8 }}>
                      ...
                      <Button
                        ...
                        variant='danger'
                        onClick={() => {
                          const newAlts = newExam.questions[0].alternatives.filter(
                            (_, i) => i !== idx
                          );
                          setNewExam({
                            ...newExam,
                        ...
                        questions: [
                          {
                            ...newExam.questions[0],
                            alternatives: [
                              ...newExam.questions[0].alternatives,
                              { description: '', isTrue: false },
                            ],
                          },
                        ],
                      });
